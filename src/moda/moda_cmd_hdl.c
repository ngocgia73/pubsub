...
void moda_ipc_register(void)
{
    int i;
   // step 1: register msg and their handler
   for(i = 0; i < sizeof(moda_ipc_cmdlist)/ sizeof(IPC_SK_MSG_T); i++)
   {
        cmd_sk_register_msg_handle(IPC_PORT, &moda_ipc_cmdlist[i]);
   }

   // step 2: register module to ipc socket server
   cmd_sk_register_module(IPC_PORT, "moda");
